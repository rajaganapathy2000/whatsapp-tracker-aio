...
export default function App() {
  ...
  // --- UPDATED: THEME AND PRIVACY STATES ---
  const [theme, setTheme] = useState(() => localStorage.getItem('waTrackerTheme') || 'light-classic');
  const [isPrivacyMode, setIsPrivacyMode] = useState(() => localStorage.getItem('waTrackerPrivacy') === 'true');
  const [isDarkMode, setIsDarkMode] = useState(false); // Legacy support for internal logic
  ...
  useEffect(() => {
    ...
    if (savedConfig) {
      ...
      // Removed old isDarkMode sync to favor new Theme Engine
    }
  }, []);

  // --- UPDATED: THEME ENGINE PERSISTENCE ---
  useEffect(() => {
    localStorage.setItem('waTrackerConfig', JSON.stringify({ apiConfig }));
    localStorage.setItem('waTrackerTheme', theme);
    localStorage.setItem('waTrackerPrivacy', isPrivacyMode);

    // Update document class for Tailwind dark variants
    const isDark = theme.startsWith('dark');
    setIsDarkMode(isDark);
    if (isDark) document.documentElement.classList.add('dark');
    else document.documentElement.classList.remove('dark');

    // Set Data Attribute for CSS Variable injection
  ...
  return (
    <div className={`${isDarkMode ? 'dark' : ''}`}>
      <div className="flex flex-col h-[100dvh] theme-bg max-w-md mx-auto text-gray-900 dark:text-gray-100 font-sans antialiased overflow-hidden sm:glass-panel sm:rounded-[3rem] sm:h-[850px] sm:my-8 relative transition-colors duration-300">
        <div className="flex-1 overflow-y-auto pb-32 scrollbar-hide z-10">
          {viewingTarget ? (
            ...
            <DashboardView 
              targets={targets} pingStats={pingStats} isSyncing={isSyncing} 
            ...
            <SettingsView 
              ...
              handleAddTarget={handleAddTarget} pingStats={pingStats} 
            />
          )}
        </div>
  ...
  return (
    <div className="p-6 pt-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-start mb-4">
        <h1 className="text-4xl font-extrabold tracking-tight theme-text">Tracker</h1>
        <div className="flex flex-col items-end">
          <div className="flex space-x-2 mb-1">
            {/* UPDATED: PRIVACY TOGGLE BUTTON */}
            <button onClick={() => setIsPrivacyMode(!isPrivacyMode)} className={`p-2 rounded-full transition-all ${isPrivacyMode ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-800 text-gray-500'}`}>
      ...
      <div className="flex items-center space-x-2 mb-6">
        ...
        <div className="relative shrink-0">
          <select 
             ...
             className="glass-card pl-10 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none theme-text font-medium cursor-pointer"
          >
...
const TargetDetailView = memo(function TargetDetailView({ target, onClose, onRemove, onTogglePin, onToggleMute, onSnooze, mongoFetch, apiConfig, isPrivacyMode }) {
  ...
  return (
    <div className="min-h-full animate-in slide-in-from-right-8 duration-300 relative z-10">
      <div className="theme-bg pt-12 pb-6 px-6 sticky top-0 z-20 border-b theme-border shadow-sm">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className={`w-16 h-16 rounded-full flex items-center justify-center border-2 ${target.isOnline ? 'border-green-500 shadow-sm' : 'theme-border'} bg-white dark:bg-gray-800 relative transition-all duration-500`}>
              <span className={`text-2xl font-bold text-gray-500 dark:text-gray-300 ${isPrivacyMode ? 'blur-sm' : ''}`}>{target.name.charAt(0).toUpperCase()}</span>
              {target.isOnline && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-30"></span>}
            </div>
            <div>
              <h1 className={`text-2xl font-bold theme-text leading-tight ${isPrivacyMode ? 'blur-md select-none' : ''}`}>{target.name}</h1>
      ...
      <div className="p-6 pb-24 space-y-6 relative">
        {isLoadingAnalytics && <div className="absolute inset-0 theme-bg/80 z-20 flex items-center justify-center m-6 rounded-3xl"><RefreshCw className="animate-spin text-blue-500" size={32} /></div>}
        ...
        <div className="grid grid-cols-4 gap-3 relative z-10">
          ...
          <button onClick={() => setShowSnoozeMenu(!showSnoozeMenu)} className={`flex flex-col items-center justify-center p-3 rounded-2xl glass-card text-purple-600 dark:text-purple-400 hover:bg-purple-50 dark:hover:bg-gray-700 transition-all active:scale-95 relative`}>
            ...
            {showSnoozeMenu && (
              <div className="absolute top-full left-0 mt-2 bg-white dark:bg-gray-800 border theme-border rounded-xl w-32 py-1 z-50 text-left shadow-lg">
          ...
          <div className="space-y-3 mt-4 max-h-64 overflow-y-auto pr-2 scrollbar-hide">
            {localStats.sessionLogs.length === 0 ? (
              ...
              localStats.sessionLogs.map((log, i) => (
                <div key={log.id || i} className="flex justify-between items-center p-3 rounded-2xl bg-gray-50 dark:bg-gray-900 border theme-border hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors">
  ...
  return (
    <div onClick={onClick} className="glass-card p-4 flex items-center active:scale-[0.98] transition-transform duration-200 cursor-pointer group hover:bg-gray-50 dark:hover:bg-gray-700">
      ...
      <div className="relative mr-4">
        <div className={`w-14 h-14 rounded-full flex items-center justify-center border-2 ${target.isOnline ? 'border-green-500' : 'theme-border'} bg-gray-100 dark:bg-gray-800 relative transition-colors`}>
          <span className={`text-lg font-bold text-gray-500 dark:text-gray-400 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors ${isPrivacyMode ? 'blur-sm' : ''}`}>{target.name.charAt(0).toUpperCase()}</span>
          {target.isOnline && <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-green-400 opacity-30"></span>}
        </div>
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center">
            <h3 className={`font-bold text-lg truncate pr-2 theme-text ${isPrivacyMode ? 'blur-md select-none' : ''}`}>{target.name}</h3>
            {target.isMuted && <BellOff size={12} className="text-gray-400" />}
        </div>
        <p className="text-sm text-gray-600 dark:text-gray-400 truncate mt-0.5">
            {target.isOnline ? 
                <span className="text-green-600 dark:text-green-400 font-bold tracking-wider text-xs uppercase">Online Now</span> 
                : <span className="font-mono">+{maskNumber(target.number)}</span>}
        </p>
      </div>
...
const CompareView = memo(function CompareView({ targets, mongoFetch, apiConfig }) {
  ...
  return (
    <div className="p-6 pt-12 animate-in fade-in slide-in-from-bottom-4 duration-500 h-[100dvh] flex flex-col relative z-10">
      <h1 className="text-4xl font-extrabold tracking-tight theme-text mb-6">Compare</h1>
      ...
      <div className="glass-card p-4 mb-6 space-y-4">
        <div className="flex items-center justify-between space-x-4">
           ...
           <select className="flex-1 theme-card theme-border rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 appearance-none theme-text cursor-pointer" value={targetA} onChange={e => setTargetA(e.target.value)}>
        ...
        <div className="flex items-center justify-between space-x-4">
           ...
           <select className="flex-1 theme-card theme-border rounded-xl px-4 py-3 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-purple-500 appearance-none theme-text cursor-pointer" value={targetB} onChange={e => setTargetB(e.target.value)}>
      ...
      <div className="glass-card p-5 flex-1 relative overflow-hidden flex flex-col mb-16">
        {isLoading && <div className="absolute inset-0 theme-bg/80 z-20 flex items-center justify-center"><RefreshCw className="animate-spin text-blue-500" size={32} /></div>}
...
const SettingsView = memo(function SettingsView({ apiConfig, setApiConfig, theme, setTheme, newTarget, setNewTarget, handleAddTarget, pingStats }) {
  // UPDATED: THEME BUTTON HELPER
  const ThemeBtn = ({ id, label, icon: Icon }) => (
    <button 
      onClick={() => setTheme(id)} 
      className={`flex flex-col items-center justify-center p-4 rounded-2xl border-2 transition-all active:scale-95 ${theme === id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 theme-text' : 'theme-border bg-gray-50 dark:bg-gray-800/50 text-gray-400'}`}
    >
      <Icon size={24} className={theme === id ? 'text-blue-500' : ''} />
  ...
  return (
    <div className="p-6 pt-12 animate-in fade-in slide-in-from-bottom-4 duration-500 relative z-10 mb-24">
      <h1 className="text-4xl font-extrabold tracking-tight theme-text mb-6">Settings</h1>
      
      {/* UPDATED: 4-WAY THEME ENGINE GRID */}
      <div className="mb-6">
        <h3 className="text-sm font-bold text-gray-600 dark:text-gray-400 uppercase tracking-widest mb-4 ml-4">Advanced Theme Engine</h3>
        <div className="grid grid-cols-2 gap-3">
          <ThemeBtn id="light-classic" label="Classic Light" icon={Sun} />
          <ThemeBtn id="light-system" label="System Light" icon={Palette} />
          <ThemeBtn id="dark-classic" label="Classic Dark" icon={Moon} />
          <ThemeBtn id="dark-amoled" label="AMOLED Black" icon={Zap} />
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="glass-card p-4">
          <div className="flex flex-col sm:flex-row gap-3">
            <input type="text" placeholder="Name" value={newTarget.name} onChange={(e) => setNewTarget({...newTarget, name: e.target.value})} className="flex-1 min-w-0 theme-card theme-border rounded-xl px-4 py-3 text-sm theme-text focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors placeholder-gray-500" />
            <input type="tel" placeholder="Number" value={newTarget.number} onChange={(e) => setNewTarget({...newTarget, number: e.target.value})} className="flex-1 min-w-0 theme-card theme-border rounded-xl px-4 py-3 text-sm theme-text focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors placeholder-gray-500" />
      ...
      <div className="mb-8">
        ...
        <div className="glass-card overflow-hidden divide-y theme-divider">
          <div className="p-4 theme-bg"><p className="text-xs font-bold uppercase tracking-wider mb-1 text-gray-600 dark:text-gray-400">1. Vercel Database Proxy</p></div>
