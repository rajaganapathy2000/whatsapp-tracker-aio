...
async function connectToWhatsApp(loginMethod = 'qr', loginNumber = '') {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        // Intercept when WhatsApp server says it is ready for authentication (provides QR payload)
        if (qr) {
            if (loginMethod === 'qr') {
                console.log('\n[WA-AUTH] 📱 Scan this QR code with your secondary WhatsApp:\n');
                qrcode.generate(qr, { small: true }); // Manually draw QR to avoid the Baileys warning
                if (db) {
                    try {
                        await db.collection('system_config').updateOne(
                            { _id: 'bot_status' },
