...
async function connectToWhatsApp(loginMethod = 'qr', loginNumber = '') {
    ...
    const sockOptions = {
        ...
        browser: Browsers.ubuntu('Chrome'), // Ubuntu Linux footprint avoids 428 on Termux
    ...
    let pairingCodeRequested = false; // Prevent spamming the API

    sock.ev.on('connection.update', async (update) => {
        const { connection, lastDisconnect, qr } = update;
        
        // Only request login codes when the socket is 100% open and ready
        if (qr) {
            if (loginMethod === 'qr') {
            ...
            } else if (loginMethod === 'pairing' && !pairingCodeRequested) {
                pairingCodeRequested = true;
                console.log('\n[WA-AUTH] ⏳ Connection secure! Requesting pairing code from WhatsApp servers...');
                try {
